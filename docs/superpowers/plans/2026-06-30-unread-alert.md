# Unread Alert System Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a single-file PHP system that polls ~10 Facebook Pages (Meta Graph API) for unread Messenger messages and sends formatted Telegram alerts, with a 5-step setup wizard and admin dashboard — no manual file editing ever needed.

**Architecture:** All logic lives in `index.php` routed via `?action=` query param. Settings (tokens, keys, cache) persist in `settings.json` blocked from web access by `.htaccess`. The cPanel cron triggers `?action=run` which fetches unread counts, caches them in `settings.json`, and POSTs a formatted message to Telegram Bot API. A `?action=api` endpoint returns the cached result as JSON for third-party systems. The first visit renders a 5-step wizard; subsequent visits show the dashboard.

**Tech Stack:** PHP 7.4+, Meta Graph API v19.0, Telegram Bot API, vanilla CSS + JS (zero dependencies), JSON file storage.

---

### Task 1: Project skeleton — routing, settings R/W, .htaccess

**Files:**
- Create: `index.php`
- Create: `.htaccess`

- [ ] **Step 1: Create .htaccess to protect settings.json**

Create `c:\xampp\htdocs\unreadAlert\.htaccess`:
```apache
<Files "settings.json">
    Order allow,deny
    Deny from all
...
- [ ] **Step 2: Create index.php with constants, settings functions, and router stub**

Create `c:\xampp\htdocs\unreadAlert\index.php`:
```php
<?php
...
Visit `http://yourdomain.com/unreadAlert/index.php`.
Expected: Blank white page (no PHP errors, render functions not defined yet — that's OK).

Verify `.htaccess` works:
Visit `http://yourdomain.com/unreadAlert/settings.json`.
Expected: 403 Forbidden.

- [ ] **Step 4: Commit**
```bash
git add index.php .htaccess
git commit -m "feat: project skeleton with routing, settings R/W, and .htaccess protection"
```

---

### Task 2: HTTP helpers and Meta Graph API client

**Files:**
- Modify: `index.php` — add after `jsonResponse()` function, before the router

- [ ] **Step 1: Add HTTP helpers and Meta API functions**

Add this block after `jsonResponse()`:
```php
// ─── HTTP ────────────────────────────────────────────────────
...
```bash
git add index.php
git commit -m "feat: add HTTP helpers and Meta Graph API client with pagination"
```

---

### Task 3: Telegram notifier and test handler

**Files:**
- Modify: `index.php` — add after Meta API section

- [ ] **Step 1: Add Telegram functions and test handler**
...
```bash
git add index.php
git commit -m "feat: add Telegram notifier with HTML formatting and Business Manager inbox URLs"
```

---

### Task 4: Office hours check, cron runner, REST API, settings save

**Files:**
- Modify: `index.php` — add after Telegram section

- [ ] **Step 1: Add office hours function**
...
- [ ] **Step 4: Add settings save handler and getkeys handler**
...
```bash
git add index.php
git commit -m "feat: add office hours, cron runner, REST API, and settings save handler"
```

---

### Task 5: Setup wizard UI (5-step)

**Files:**
- Modify: `index.php` — add `renderSetupWizard()` before the router

- [ ] **Step 1: Add renderSetupWizard() function**

Add this entire function before the `$action = ...` line at the bottom:
...
<div class="wizard">
  ...
  <div class="wb">
    ...
    <div class="step" id="step5">
      ...
      <p style="text-align:center;margin-bottom:20px">Add this command to cPanel Cron Jobs (every 15 minutes by default):</p>
      <div class="fg">
...
function testTelegram() {
  ...
  fetch(BASE+'?action=test_telegram',{method:'POST',body:fd})
    .then(r=>r.json())
    .then(d=>{st.className='st '+(d.ok?'success':'error');st.textContent=d.ok?'✅ Test message sent! Check your channel.':'❌ '+d.message;});
}
...
function addPage() {
  ...
  div.innerHTML=`<div class="prh"><span class="prt">Page #${n}</span>
...
function verPage(n) {
  ...
  fetch(BASE+'?action=verify_page',{method:'POST',body:fd})
    .then(r=>r.json())
    .then(d=>{st.className='st '+(d.ok?'success':'error');st.textContent=d.ok?'✅ Verified: '+d.name:'❌ '+d.message;});
}
...
Delete `settings.json` if it exists, then visit `http://yourdomain.com/unreadAlert/index.php`.
Expected:
- Beautiful dark-themed 5-step wizard renders
- Step 1 shows two pre-filled keys with Regenerate buttons
- Next → advances to Step 2 (validates Telegram fields are not empty)
- Telegram test button fires AJAX and shows success/error badge
- Step 3 starts with one page row; "+" adds more; Verify button checks token
- Step 4 shows interval buttons (15min active by default) and 24/7 toggle
- Clicking Next on Step 4 submits the form, then Step 5 shows cron command + API URL with Copy buttons
- "Go to Dashboard →" leads to dashboard

- [ ] **Step 3: Commit**
```bash
git add index.php
git commit -m "feat: add 5-step setup wizard with live Telegram test and page token verification"
```

---

### Task 6: Admin dashboard and settings page UI

**Files:**
- Modify: `index.php` — add shared layout helpers, dashboard, and settings page before the router

- [ ] **Step 1: Add shared CSS/layout helper functions**

Add before the router:
```php
// ─── UI: SHARED ──────────────────────────────────────────────
...
- [ ] **Step 2: Add renderDashboard() function**
...
<div class="card">
  ...
  <table>
    ...
    <tr>
      ...
      <td><?= (int)$p['unread_count'] > 0 ? '<span class="badge br">'.($p['unread_count']).'</span>' : '<span class="badge bg">0</span>' ?></td>
...
- [ ] **Step 3: Add renderSettingsPage() function**
...
function addPg(){
  pc++;const n=pc;
  const d=document.createElement('div');d.className='pr';d.id='pr'+n;
...
- [ ] **Step 4: Verify dashboard and settings page**

After completing the setup wizard once:
- Visit `http://yourdomain.com/unreadAlert/index.php` — dashboard shows stat cards, page table, Run Now button
- Visit `http://yourdomain.com/unreadAlert/index.php?view=settings` — settings form loads with all saved values
- Change interval, save — redirects back to settings; change is preserved
- "Run Now" on dashboard fires cron inline and refreshes after 1.5s

- [ ] **Step 5: Commit**
```bash
git add index.php
git commit -m "feat: add admin dashboard and settings page with full edit support"
```
...
- [ ] **Step 1: Full setup walkthrough**

1. Delete `settings.json` if it exists
2. Visit `index.php` → setup wizard appears
3. Step 1: keep auto-generated keys
4. Step 2: enter real bot token and channel ID → click "Send Test Message" → confirm it arrives in Telegram
5. Step 3: add one real page → click "Verify" → confirm page name appears
6. Step 4: choose 15 min, 24/7
7. Next → settings save → Step 5 shows correct cron command and API URL

- [ ] **Step 2: Manual cron run**

Click "Go to Dashboard" → click "Run Now".
Expected: "✅ Alert sent successfully" (if unread > 0) or "No unread messages, notification skipped".
Check Telegram — if any unread messages, the formatted alert should appear with page name, count, and Business Manager inbox link.

- [ ] **Step 3: Verify REST API**

```bash
curl "https://yourdomain.com/unreadAlert/index.php?action=api&key=YOUR_API_KEY"
```
Expected response:
```json
{
...
```bash
curl "https://yourdomain.com/unreadAlert/index.php?action=api&key=wrongkey"
```
Expected: `{"status":"error","message":"Unauthorized"}` with HTTP 401.

```bash
curl "https://yourdomain.com/unreadAlert/settings.json"
```
Expected: 403 Forbidden.

- [ ] **Step 5: Set up cPanel cron job**

In cPanel → Cron Jobs, add:
- Command: the cron command from Step 5 of the wizard (also visible in Settings page)
- Interval: Every 15 Minutes (`*/15 * * * *`)

- [ ] **Step 6: Final commit**
```bash
git add .
git commit -m "feat: unread alert system complete — single-file PHP with wizard, dashboard, API"
```
